  ...
  return (
    <section className="aurora-glass flex min-h-0 flex-1 flex-col overflow-hidden rounded-aurora-2xl border border-white/10 p-3.5 sm:p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        ...
        <div className="flex rounded-aurora-lg border border-white/10 bg-white/3 p-1 text-xs">
          <button
            type="button"
            onClick={() => setMode("memory")}
            className={`rounded-aurora-md px-2 py-1 ${mode === "memory" ? "bg-white/12 text-white" : "text-white/55 hover:text-white/90"}`}
          >
            Memory
          </button>
          <button
            ...
            className={`rounded-aurora-md px-2 py-1 ${mode === "files" ? "bg-white/12 text-white" : "text-white/55 hover:text-white/90"}`}
          >
            Files
          </button>
        </div>
      ...
      <div className="mb-3 flex gap-2">
        <input
          ...
          className="h-11 min-w-0 flex-1 rounded-aurora-lg border border-white/12 bg-black/30 px-3 text-sm text-white outline-none ring-aurora-cyan/25 focus:ring-2"
        />
        <button
          type="button"
          onClick={() => void runSearch()}
          disabled={loading || !query.trim()}
          className="inline-flex h-11 items-center gap-2 rounded-aurora-lg border border-cyan-300/25 bg-cyan-400/10 px-3 text-sm font-medium text-cyan-100 transition hover:bg-cyan-400/16 disabled:opacity-50"
        >
